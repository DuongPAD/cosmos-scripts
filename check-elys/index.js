...
async function checkAirdropElysCoin() {
  try {
      ...
      try {
        ...
        if (response.data) {
          ...
          if (totalForUser > 0) {
            ...
            elysWallets.push({ address: elys, amount: totalForUser })
          }
          csvRows.push(
            `Oliver ${i + 1},${atom},${elys},${totalForUser.toFixed(2)}`
          )
          console.log(
            `Oliver ${i + 1}: ${elys}, Total for user: ${totalForUser.toFixed(
              2
            )}`
          )
        } else {
    ...
    console.error(
      'An error occurred while reading the file or parsing JSON:',
      error
    )
  }
}
